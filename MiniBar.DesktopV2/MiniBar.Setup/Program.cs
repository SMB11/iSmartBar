...
namespace MiniBar.Setup
{
    class Program
    {
        static List<string> modules = new List<string> { "MiniBar.ProductsModule.dll", "MiniBar.ProductsModule.dll.config", "MiniBar.ConfigurationModule.dll", "MiniBar.ConfigurationModule.dll.config" };
        ...
        static public void Main(string[] args)
        {
            ...
            var project = new Project("MiniBar Admin",
                new Dir(@"%ProgramFiles%/MyBar/Admin",
                            ...
                            new Dir(@"%ProgramMenu%/mybar",
                                              new ExeFileShortcut("MiniBar Admin", "[INSTALLDIR]" + "MiniBarManager.exe", "") { WorkingDirectory = "INSTALLDIR" }
                                          ),
                            new Dir("%Desktop%",
                                    new ExeFileShortcut("MiniBar Admin", "[INSTALLDIR]" + "MiniBarManager.exe", "") { WorkingDirectory = "INSTALLDIR" }
                            ),
    ...
    public class CustomActions
    {
        ...
        {
            MessageBox.Show(session.GetTargetPath("de"), "Embedded Managed CA");
