...
using Core.Documents.Extensions;
using Core.Documents.Metadata;
...
namespace Core.Documents.Excel
{
        ...
        where T: class, new()
    {
        ...
        public ExcelDocument(DocumentAdapter<T> adapter)
        {
                ...
                try
                {
                    foreach (ColumnMetadata column in DocumentAdapter.Columns)
                    {
                        if (column.ColumnType == ColumnType.Normal)
                        {
                            ...
                            string error = DevExpress.Mvvm.IDataErrorInfoHelper.GetErrorText(obj, column.PropertyInfo.Name);
                            ...
                            if(column.PropertyInfo2 != null)
                            {
                                ...
                                error = DevExpress.Mvvm.IDataErrorInfoHelper.GetErrorText(obj, column.PropertyInfo2.Name);
                                if (!String.IsNullOrEmpty(error))
                                    throw new ExcelParseException(currentColumn, rowCounter, error);
                            }
                        }
